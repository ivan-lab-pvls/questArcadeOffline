...
class Game{
  ...
  static bool _checkWinner(bool isWhite){
    bool win = true;
    for (var element in _area) {
      if(element!=null&&element.isSecond!=isWhite){
        win=false;
        break;
      }
    }
  ...
  static bool _someoneCanMove(bool isWhite){
    bool result = false;
    for (var element in _area) {
      if(element!=null&&element.isSecond==isWhite){
  ...
  static bool _someoneCanKill(bool isWhite){
    bool result = false;
    for (var element in _area) {
      if(element!=null&&element.isSecond==isWhite){
  ...
  static List<String> get logMessage=>_log.logs();

  static String select(int x, int y){
    var selected = _area[x*8+y];

    if(selected!.isSecond!=isWhiteTurn&&_canReselect){
...
class GameLog{
  ...
  void setWhoTurn(bool isWhite){
    turn=isWhite?"White":"Blue";
  }

  void setWinner(bool isWhite){
    winner=isWhite?"White":"Blue";
    message="Game Over";
  }

  List<String> logs(){

    var result = ['$winner', '$turn', '$message'];
